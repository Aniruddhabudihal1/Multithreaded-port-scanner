...
int host_is_up(char *address, int quick_check) {
  ...
  timeout.tv_usec =
      quick_check ? 100000 : 300000; // 100 microseconds or 300 microseconds

  // here we are trying to connect to fewcommon ports to check: 80 (HTTP), 443
  // (HTTPS), 22 (SSH), 21 (FTP)
...
void defaultScan() {
  ...
  int num_threads = 20; // Number of threads for host discovery
